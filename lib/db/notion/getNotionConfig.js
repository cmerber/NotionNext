...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  await hydrateMissingConfigRows(data.pageRecordMap, data.content)
...
async function hydrateMissingConfigRows(pageRecordMap, content) {
  ...
  const block = pageRecordMap.block || (pageRecordMap.block = {})
  const missingPageIds = rowPageIds.filter(id => !block[id])
  if (missingPageIds.length > 0) {
    Object.assign(block, await fetchInBatches(missingPageIds))
  }
}
