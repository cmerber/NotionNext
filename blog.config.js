...
const BLOG = {
  ...
  NEXT_REVALIDATE_SECOND: process.env.NEXT_PUBLIC_REVALIDATE_SECOND || 3600, // 个人博客默认缓存1小时；减少Notion重复抓取和ISR重建，发布后可通过重新部署或按需刷新立即更新。
  ...
  BLOG_FAVICON: process.env.NEXT_PUBLIC_FAVICON || null, // blog favicon；未单独配置时自动使用 Notion 首页图标
  PWA_ENABLE:
    process.env.NEXT_PUBLIC_PWA_ENABLE !== undefined
      ? process.env.NEXT_PUBLIC_PWA_ENABLE === 'true'
      : true, // 默认启用；可通过环境变量显式关闭
  PWA_NAME: process.env.NEXT_PUBLIC_PWA_NAME || '橙子星球',
  PWA_SHORT_NAME: process.env.NEXT_PUBLIC_PWA_SHORT_NAME || 'QCODE',
  ...
  PWA_THEME_COLOR: process.env.NEXT_PUBLIC_PWA_THEME_COLOR || '#4f65f0',
  PWA_BACKGROUND_COLOR:
    process.env.NEXT_PUBLIC_PWA_BACKGROUND_COLOR || '#f7f9fe',
